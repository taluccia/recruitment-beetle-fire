...
qplot(seedling.ct, data = fire2, main = "Seedling count")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(fire.type, seedling.ct, data = fire2, main = "Seedling Count by fire type")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.bom, data = fire2, main = "Proportion of beetle-killed trees")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.prefire, data = fire2, main = "Proportion of prefire-killed trees")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.canopy, data = fire2, main = "Proportion of Stems with Branches")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.nocanopy, data = fire2, main = "Proportion of stems with no Canopy")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.opencone, data = fire2, main = "Proportion of stems with open cones")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.closedcone, data = fire2, main = "Proportion of stems with closed cones")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.cone.ct, data = fire2, main = "Proportion of of cones per stems")
```

```{r, echo=FALSE}
qplot(y= seedling.ct, x= prop.cone.ct, color = fire.type, data = fire2, main = "Cones per stems by seedlings with fire type")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
ggpairs( dplyr::select(fire2, prop.bom, prop.canopy, prop.opencone, prop.cone.ct),
         upper = list(continuous = wrap("cor", size = 7) ), title="Correlation plot")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(prop.pfcwd, data = cwddata, main = "Proportion of Prefire CWD")
```

```{r, echo=FALSE, message=FALSE, warning=FALSE}
qplot(x = prop.pfcwd, y = seedling.ct, data = cwddata, main = "Proportion of Prefire CWD")
```
...
Explanatory varaible is the proportion of open cones
...
The explanatory variable is the proportion of beetle killed trees.
...
## Prefire mortality model

The explanatory variable is the proportion of all prefire killed trees. This inclueds both beetle killed and the 7% of trees that were killed prefire by an agent, but could not be specifically linked to mountain pine beetles 
...
## Exposed mineral soil model

The explanatory variable is the proportion of exposed mineral soil. 
...
The explanatory variable is the proportion of prefire CWD.
...
Supplemental figures with raw data points and plot each fire severity class separately.
